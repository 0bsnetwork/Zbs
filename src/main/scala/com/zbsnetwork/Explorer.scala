...
object Explorer extends ScorexLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val configFilename = Try(args(0)).toOption.getOrElse("zbs-testnet.conf")

    val settings = ZbsSettings.fromConfig(loadConfig(ConfigFactory.parseFile(new File(configFilename))))
      ...
      flag match {
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.zbsBalance(addressId)(h)

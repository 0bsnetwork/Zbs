...
import com.zbsnetwork.transaction.smart.script.ContractScript.ContractScriptImpl
import com.zbsnetwork.transaction.smart.script.ScriptRunner.TxOrd
import com.zbsnetwork.transaction.smart.script.{Script, ScriptRunner}
import com.zbsnetwork.transaction.smart.{ContractInvocationTransaction, ZbsEnvironment}
      ...
      val ctx = Monoid
        .combineAll(
          Seq(
            ...
            ZbsContext.build(StdLibVersion.V3,
                             new ZbsEnvironment(AddressScheme.current.chainId, Coeval(tx.asInstanceOf[In]), Coeval(height), blockchain),
                             false)
          ))
        .evaluationContext

...
import com.zbsplatform.settings.{NetworkSettings, loadConfig}
...
class BlacklistParallelSpecification extends FeatureSpec with GivenWhenThen with ParallelTestExecution {
  ...
  private val config = loadConfig(ConfigFactory.parseString("""zbs.network {
      |  known-peers = []
      |  file = null
      |  black-list-residence-time: 1s
      |}""".stripMargin))

  private val networkSettings = config.as[NetworkSettings]("zbs.network")

...
class OrderBookSpec extends FreeSpec with Matchers with MatcherTestData with NTPTime {
  ...
  "partially execute order with zero fee remaining part" in {
    val ord1 = sell(pair, 1500 * Constants.UnitsInZbs, 0.0006999)
  ...
  "partially execute order with price > 1 and zero fee remaining part " in {
    ...
    val ord1 = sell(pair, (0.1 * Constants.UnitsInZbs).toLong, 1850)
    val ord2 = sell(pair, (0.01 * Constants.UnitsInZbs).toLong, 1840)
    val ord3 = buy(pair, (0.0100001 * Constants.UnitsInZbs).toLong, 2000)

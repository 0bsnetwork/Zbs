...
import com.zbsnetwork.matcher.model.Events.{OrderAdded, OrderCanceled}
import com.zbsnetwork.matcher.model._
import com.zbsnetwork.matcher.{MatcherSettings, MatcherTestData}
...
class OrderValidatorSpecification
  ...
  "OrderValidator" should {
    "allow buying ZBS for BTC without balance for order fee" in
  ...
  private def newBuyOrder: Order =
    buy(pair = pairZbsBtc, amount = 100 * Constants.UnitsInWave, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInWave).toLong))

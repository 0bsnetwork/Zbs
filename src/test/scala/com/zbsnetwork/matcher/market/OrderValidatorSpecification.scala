...
import com.zbsnetwork.matcher.model.OrderValidator.ValidationResult
import com.zbsnetwork.matcher.model._
import com.zbsnetwork.settings.Constants
import com.zbsnetwork.state.diffs.{CommonValidation, produce}
...
import com.zbsnetwork.{NoShrink, TestTime, WithDB}
...
class OrderValidatorSpecification
  ...
  "OrderValidator" should {
    "allow buying ZBS for BTC without balance for order fee" in asa() { v =>
      v shouldBe 'right
    }
    ...
    "reject new order" when {
      ...
      "order price has invalid non-zero trailing decimals" in forAll(assetIdGen(1), accountGen, Gen.choose(1, 7)) {
        case (Some(amountAsset), sender, amountDecimals) =>
          portfolioTest(Portfolio(11 * Constants.UnitsInZbs, LeaseBalance.empty, Map.empty)) { (ov, bc) =>
    ...
    "validate order with smart token" when {
      ...
      val portfolio = Portfolio(10 * Constants.UnitsInZbs,
                                LeaseBalance.empty,
                                Map(
                                  asset1 -> 10 * Constants.UnitsInZbs,
                                  asset2 -> 10 * Constants.UnitsInZbs
                                ))
  ...
  private def newBuyOrder: Order =
    buy(pair = pairZbsBtc, amount = 100 * Constants.UnitsInZbs, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInZbs).toLong))
  ...
  private def mkOrderValidator(bc: Blockchain, tc: ExchangeTransactionCreator) =
    OrderValidator.blockchainAware(bc,
                                   ...
                                   (0.003 * Constants.UnitsInZbs).toLong,
                                   MatcherAccount,
                                   ntpTime,

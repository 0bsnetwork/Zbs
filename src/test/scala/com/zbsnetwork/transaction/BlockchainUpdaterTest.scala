...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithState {
  ...
  private val ZbsSettings = history.DefaultZbsSettings.copy(
    blockchainSettings = history.DefaultZbsSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultZbsSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultZbsSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val ZbsSettingsWithDoubling = ZbsSettings.copy(
    blockchainSettings = ZbsSettings.blockchainSettings.copy(
      functionalitySettings = ZbsSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(ZbsSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(ZbsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(ZbsSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(ZbsSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater

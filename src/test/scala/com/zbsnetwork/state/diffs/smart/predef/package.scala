...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithPureFunctions(tx, t))}
       |${dropLastLine(scriptWithZbsFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && zbs else crypto""".stripMargin
  ...
  def scriptWithZbsFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Zbs context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && zbsBalance(tx.sender) != 0
       |
       | let zbs = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | zbs""".stripMargin

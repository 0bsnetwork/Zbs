...
import com.zbsnetwork.transaction.assets.{IssueTransactionV1, IssueTransactionV2, SponsorFeeTransaction}
...
import com.zbsnetwork.transaction.{GenesisTransaction, Transaction, ValidationError}
    ...
    for {
      ...
      val transferAssetTx = TransferTransactionV1
        .selfSigned(
          ...
          ts,
          None,
          if (smartToken) { 1 * Constants.UnitsInZbs + ScriptExtraFee } else { 1 * Constants.UnitsInZbs },
          ...
          Seq(
            SponsorFeeTransaction
              .selfSigned(richAcc, issueTx.id(), Some(10), if (smartToken) {
                Constants.UnitsInZbs + ScriptExtraFee
              } else {
                Constants.UnitsInZbs
              }, ts)
              .explicitGet()

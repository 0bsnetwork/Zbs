...
import com.zbsnetwork.lang.v1.compiler.{CompilerContext, ExpressionCompilerV1}
...
import com.zbsnetwork.utils.functionCosts
...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createZbsTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createZbsTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()

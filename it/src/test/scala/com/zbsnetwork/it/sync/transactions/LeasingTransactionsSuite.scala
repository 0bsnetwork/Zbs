...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make leasing without having enough balance") {
    ...
    assertBadRequestAndResponse(sender.lease(secondAddress, firstAddress, balance2 + 1.zbs, minFee), "Reason: Cannot lease more than own")
  ...
  test("can not make leasing without having enough zbs for fee") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("lease cancellation reverts eff.b. changes; lessor pays fee for both lease and cancellation") {
    import com.zbsplatform.transaction.lease.LeaseTransaction.Status._
  ...
  test("can not make leasing without having enough your zbs to self") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.lease(firstAddress, firstAddress, balance1 + 1.zbs, minFee), "Transaction to yourself")
    nodes.waitForHeightArise()

...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make mass transfer without having enough zbs") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative zbs balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.zbs), Transfer(secondAddress, 2.zbs), Transfer(thirdAddress, 3.zbs))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.zbs)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.zbs)

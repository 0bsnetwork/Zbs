...
import com.zbsnetwork.it.sync.{script, someAssetAmount, _}
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's zbs balance is decreased by fee") {
  ...
  test("cannot transact without having enough zbs") {
    val (balance, eff) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative zbs balance")
    nodes.waitForHeightArise()
    notMiner.assertBalances(firstAddress, balance, eff)

    val leaseAmount = 1.zbs

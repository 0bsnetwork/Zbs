package com.zbsnetwork.it.sync.utils

import com.zbsnetwork.account.{Address, AddressScheme, Alias, PublicKeyAccount}
import com.zbsnetwork.it.transactions.BaseTransactionSuite
import com.zbsnetwork.state.{BinaryDataEntry, BooleanDataEntry, IntegerDataEntry}
import com.zbsnetwork.transaction.{CreateAliasTransactionV1, CreateAliasTransactionV2, DataTransaction, Proofs}
...
class TransactionSerializeSuite extends BaseTransactionSuite with TableDrivenPropertyChecks {
  ...
  private val transfers = MassTransferTransaction
    .parseTransfersList(List(Transfer(firstAddress, 1.zbs), Transfer(secondAddress, 2.zbs)))
    .right
    .get

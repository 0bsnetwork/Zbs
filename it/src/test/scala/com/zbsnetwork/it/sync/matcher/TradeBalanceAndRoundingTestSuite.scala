...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade WCT-USD 1" - {
    ...
    "place wct-usd order" in {
      ...
      val expectedReservedZbs = matcherFee - LimitOrder.partialFee(matcherFee, wctUsdSellAmount, executedAmount)
  ...
  "Submitted order Canceled during match" in {
    ...
    matcherNode.waitOrderStatusAndAmount(zbsUsdPair, aliceOrderId, "Cancelled", Some(0), 1.minute)

...
import com.zbsnetwork.transaction.assets.exchange.{AssetPair, ExchangeTransactionV2, Order}
...
package object smartcontract {
  ...
  def zbsContextScript(dtx: DataTransaction, accountScript: Boolean): String =
    s"""
       | match tx {
       |  case ext : ExchangeTransaction =>
       |    # Zbs context
       ...
       |
       |     let balances = assetBalance(ext.sender, unit) > 0 && zbsBalance(ext.sender) != 0
       |

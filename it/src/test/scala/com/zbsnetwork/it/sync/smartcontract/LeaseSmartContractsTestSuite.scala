...
import com.zbsnetwork.transaction.lease.{LeaseCancelTransactionV2, LeaseTransactionV2}
...
class LeaseSmartContractsTestSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract, make leasing and cancel leasing") {
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (minFee + setScriptFee + 0.2.zbs),
                            eff1 + 9 * transferAmount - (minFee + setScriptFee + 0.2.zbs))
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.zbs),
                            eff1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.zbs))

...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false) extends AutoCloseable with ScorexLogging {
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("zbs.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val zbsNetwork: Network = {
    val networkName = s"zbs-${hashCode().toLong.toHexString}"
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val restApiPort    = actualConfig.getString("zbs.rest-api.port")
      val networkPort    = actualConfig.getString("zbs.network.port")
      val matcherApiPort = actualConfig.getString("zbs.matcher.port")
      ...
      val nodeName   = actualConfig.getString("zbs.network.node-name")
      ...
      val configOverrides: String = {
        ...
        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dzbs.network.declared-address=$ip:$networkPort $ntpServer"
      ...
      val containerConfig = ContainerConfig
        ...
        .env(s"ZBS_OPTS=$configOverrides")
        .build()

      val containerId = {
        val containerName = s"${zbsNetwork.name()}-$nodeName"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, ZbsSettings.fromConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: ZbsSettings): NodeInfo = {
    ...
    val zbsIpAddress = containerInfo.networkSettings().networks().get(zbsNetwork.name()).ipAddress()
    ...
    NodeInfo(
      ...
      new InetSocketAddress(zbsIpAddress, networkPort)
    )
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(zbsNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${zbsNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(zbsNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${zbsNetwork.name()}", e)
      }

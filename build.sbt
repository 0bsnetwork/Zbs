...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"zbs-all-${version.value}.jar",
...
inConfig(Linux)(
  Seq(
    maintainer := "zbsnetwork.com",
    packageSummary := "Zbs node",
    packageDescription := "Zbs node"
  ))

bashScriptExtraDefines += s"""addJava "-Dzbs.directory=/var/lib/${normalizedName.value}""""
...
inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"zbs-${network.value}.conf" -> "doc/zbs.conf.sample"),
    mappings := {
